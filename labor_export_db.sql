...
CREATE TABLE domestic.staff (
    ...
    staff_sex           NVARCHAR(3)     NOT NULL
                        CHECK (UPPER(staff_sex) IN ('NAM', 'NỮ')),
...
CREATE TABLE student.information (
    ...
    student_sex         NVARCHAR(3)     NOT NULL
                        CHECK (UPPER(student_sex) IN ('NAM', 'NỮ')),
...
CREATE TABLE student.dependence (
    ...
    dependence_sex      NVARCHAR(3)     NOT NULL
                        CHECK (UPPER(dependence_sex) IN ('NAM', 'NỮ')),
...
CREATE TABLE abroad.association (
    ...
    asso_name       NVARCHAR(100)   NOT NULL UNIQUE,
    asso_manager    NVARCHAR(70)    NOT NULL,
    asso_contact    VARCHAR(15)     NOT NULL UNIQUE,
    asso_email      VARCHAR(70)     DEFAULT 0 UNIQUE,
    asso_address    VARCHAR(200)    NOT NULL UNIQUE
);
...
CREATE TABLE abroad.branch (
    ...
    branch_name     NVARCHAR(70)        NOT NULL UNIQUE
);

CREATE TABLE abroad.job (
    job_id          VARCHAR(6)      PRIMARY KEY,
    job_name        NVARCHAR(70)    NOT NULL UNIQUE,
...
CREATE TABLE abroad.company (
    ...
    company_address     NVARCHAR(200)   NOT NULL UNIQUE,
    company_contact     VARCHAR(15)     NOT NULL UNIQUE,
    company_email       VARCHAR(70)     DEFAULT 0 UNIQUE,
    asso_id             VARCHAR(6)      NOT NULL,
    job_id              VARCHAR(6)      NOT NULL
);
...
CREATE TABLE abroad.worker_information (
    ...
    worker_sex          NVARCHAR(3)         NOT NULL
                        CHECK (UPPER(worker_sex) IN ('NAM', 'NỮ')),
    worker_dob          DATE                NOT NULL,
    worker_phone        VARCHAR(15)         NOT NULL UNIQUE,
    ...
    job_id              VARCHAR(6)          NOT NULL,
    worker_email        VARCHAR(70)         DEFAULT 0 UNIQUE
);
...
CREATE TABLE abroad.worker_dependence (
    ...
    dependence_sex      NVARCHAR(3)     NOT NULL
                        CHECK (UPPER(dependence_sex) IN ('NAM', 'NỮ')),
    dependence_phone    VARCHAR(15)     NOT NULL UNIQUE,
